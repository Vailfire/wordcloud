...
import { add, subtract, multiply, dot, magnitude, normalize } from './vector';
...
const multiplyTestCases = fixedParametersTestCases.map(({ caption, args }) => ({
  caption,
...
runTestCases(multiply, 'scalar multiplication with', multiplyTestCases, [vectors[0], [-1.5, 4], vectors[2], [2, 4, 6, 8]]);

// Dot product of a single vector is 0, because we can also assume an empty vector is a null vector.
runTestCases(dot, 'dot product with', fixedParametersTestCases, [0, 6.5, 0, 10]);
